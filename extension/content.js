...
// --- sites quelconques ------------------------------------------------------
// Aucun selecteur propre a un site : on part de la balise <video> elle-meme,
// seul element commun a tous les lecteurs. Son orientation decide de la forme
// a poser — bouton rond sur une video verticale, barre sous une horizontale.

const FLOAT_CLASS = "mediagrab-float";

const boxOf = (el) => el.getBoundingClientRect();
const areaOf = (el) => boxOf(el).width * boxOf(el).height;

function mainVideo() {
  const seen = [...document.querySelectorAll("video")].filter((v) => {
    const b = boxOf(v);
    return b.width >= 160 && b.height >= 90 && b.bottom > 0 && b.top < innerHeight;
  });
  // La plus grande a l'ecran : sur une page qui en empile plusieurs, c'est
  // celle que l'utilisateur regarde.
  return seen.sort((a, b) => areaOf(b) - areaOf(a))[0] || null;
}

const isPortrait = (video) => boxOf(video).height > boxOf(video).width;

/** Conteneur du lecteur : on remonte tant que le parent garde a peu pres la
 *  largeur de la video. Poser la barre sous ce bloc plutot que sous la balise
 *  evite de la glisser dans un conteneur en positionnement absolu. */
function playerContainer(video) {
  let el = video;
  const width = boxOf(video).width;
  for (let depth = 0; depth < 5 && el.parentElement; depth++) {
    const parent = el.parentElement;
    if (parent === document.body) break;
    const pw = boxOf(parent).width;
    if (pw > width * 1.4 || pw < width * 0.8) break;
    el = parent;
  }
  return el;
}

// --- bouton rond, pour les formats verticaux --------------------------------

function buildFloatButton() {
  const host = document.createElement("div");
  host.className = FLOAT_CLASS;

  const button = document.createElement("button");
  button.type = "button";
  button.title = "Telecharger avec MediaGrab";
  ...
  label.className = "mediagrab-float__label";
  label.textContent = "Telecharger";
  ...
  button.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    downloadCurrent(host);
  });
  return host;
}

/** Le bouton suit la video : elle bouge au defilement et au redimensionnement. */
function placeFloat(host, video) {
  const b = boxOf(video);
  const outside = b.right + 76 < innerWidth;
  host.style.top = `${Math.round(b.top + b.height * 0.5)}px`;
  host.style.left = `${Math.round(outside ? b.right + 14 : b.right - 74)}px`;
}

async function downloadCurrent(host) {
  const button = host.querySelector("button");
  const label = host.querySelector(".mediagrab-float__label");
...
function injectGeneric() {
  ...
  if (!video) {
    document.querySelector(`.${FLOAT_CLASS}`)?.remove();
    document.getElementById(WRAP_ID)?.remove();
    floatTarget = null;
    return;
  }
  ...
  if (isPortrait(video)) {
    document.getElementById(WRAP_ID)?.remove();
    let host = document.querySelector(`.${FLOAT_CLASS}`);
    if (!host) {
      host = buildFloatButton();
      document.body.appendChild(host);
    }
    floatTarget = video;
    placeFloat(host, video);
    return;
  }
  ...
  if (existing?.isConnected && currentVideoId === location.href) return;
  existing?.remove();

  const container = playerContainer(video);
  if (!container.parentElement) return;

  currentVideoId = location.href;
  options = [];
  selected = null;
...
// Le bouton flottant est en position fixe : il doit suivre la video.
addEventListener("scroll", () => {
  const host = document.querySelector(`.${FLOAT_CLASS}`);
  if (host && floatTarget?.isConnected) placeFloat(host, floatTarget);
}, { passive: true });
...
const ON_YOUTUBE = /(^|\.)youtube\.com$/.test(location.hostname);

function inject() {
  if (ON_YOUTUBE && isShorts()) return injectShorts();
  if (!ON_YOUTUBE) return injectGeneric();

  const id = videoIdFromUrl();
  if (!id) return injectGeneric();
